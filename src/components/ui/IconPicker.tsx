...
export default function IconPicker({ 
  ...
  const handleSelect = (val: string) => {
    onChange(val);
    const newRecent = [val, ...recentAssets.filter(a => a !== val)].slice(0, 18);
  ...
  const renderIcon = (name: string, type: 'material' | 'lucide', size = 20) => {
    ...
    const IconC = findLucide(name) || HelpCircle;
  ...
  return (
    <>
      ...
      <Modal isOpen={isOpen} onClose={() => { setIsOpen(false); }} title="Asset Library" type="custom" maxWidth="max-w-[90vw]">
            ...
            <div className="flex gap-6">
              {allowedTabs.includes('icons') && (
                <button onClick={() => setActiveTab('icons')} className={`pb-4 px-2 text-xs font-black uppercase tracking-widest transition-all border-b-2 ${activeTab === 'icons' ? 'border-[#264376] text-[#264376]' : 'border-transparent text-slate-400'}`}>
                  <div className="flex items-center gap-2"><LayoutGrid size={14} /> Icons</div>
                </button>
              )}
          ...
          <div className="flex-1 overflow-hidden">
            {activeTab === 'icons' ? (
              <div className="flex flex-col h-full gap-6">
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar pr-2 space-y-10 pb-20">
                  {recentAssets.length > 0 && (
                    <div className="space-y-4">
                      <div className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-900 px-2"><History size={14} className="text-[#264376]" /> Recently Used</div>
