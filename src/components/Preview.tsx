...
interface PreviewProps {
  ...
  typography: TypographySettings; // 传入字体设置
}
...
const Preview: React.FC<PreviewProps> = React.memo(({ page, pageIndex, totalPages, printSettings, typography }) => {
  ...
  const availWidthMm = isHorizontalBinding ? (printSettings.widthMm - printSettings.gutterMm) : printSettings.widthMm;
  const availHeightMm = !isHorizontalBinding ? (printSettings.heightMm - printSettings.gutterMm) : printSettings.heightMm;
  ...
  const canvasHeight = isPrintEnabled ? designDims.width * (printSettings.heightMm / printSettings.widthMm) : designDims.height;
