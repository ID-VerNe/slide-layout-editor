...
export default function EditorPage() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === 's') { e.preventDefault(); handleSmartSave(); return; }
      if ((e.ctrlKey || e.metaKey) && e.key === 'z') { e.preventDefault(); if (e.shiftKey) redo(); else undo(); }
      if ((e.ctrlKey || e.metaKey) && e.key === 'y') { e.preventDefault(); redo(); }
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [undo, redo, pages, projectTitle, currentFilePath, theme]);
  ...
  const handleSmartSave = async () => {
    if (!nativeFs.isElectron()) { saveToDB(previewRef, true); return; }
    const content = { version: "3.0", title: projectTitle, pages, theme, minimalCounter, customFonts, imageQuality, printSettings };
    const result = await nativeFs.saveProject(content, currentFilePath || undefined, projectTitle);
    if (result.success) { if (result.filePath) setCurrentFilePath(result.filePath); markAsSaved(); }
  };
  ...
  const handleNativeOpen = async () => {
    ...
    if (result.success && result.content) {
      try {
        ...
        initProject(projectId || 'native', project);
  ...
  const handleExport = async (format: 'png' | 'pdf') => {
    ...
    try {
      ...
      if (nativeFs.isElectron() && format === 'png' && exportScope === 'all') {
        ...
        alert(`Successfully exported ${exportIndices.length} images to ${dirResult.path}`);
      } else if (format === 'pdf') {
        ...
        for (let i = 0; i < exportIndices.length; i++) {
          ...
          links.forEach((linkEl: any) => {
            ...
            if (url) doc.link(rect.left - pageRect.left, rect.top - pageRect.top, rect.width, rect.height, { url });
          });
          setExportProgress(Math.round(((i + 1) / exportIndices.length) * 100));
        }
  ...
  if (!isLoaded || pages.length === 0) { return (<div className="h-screen w-screen bg-slate-50 flex flex-col items-center justify-center space-y-6"><div className="w-16 h-16 border-4 border-[#264376]/10 border-t-[#264376] rounded-full animate-spin" /><p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Loading SlideGrid Studio...</p></div>); }
    ...
    <div className="flex h-screen bg-neutral-100 overflow-hidden font-sans">
      ...
      <div className="flex-1 flex overflow-hidden">
        <motion.div initial={false} animate={{ flex: 1 }} className="bg-neutral-200/50 flex flex-col overflow-hidden relative">
          <TopNav projectTitle={projectTitle} setProjectTitle={setProjectTitle} fallbackTitle={fallbackTitle} currentPageIndex={currentPageIndex} totalPages={pages.length} onPageChange={setCurrentPageIndex} enforceA4={false} onToggleEnforceA4={()=>{}} previewZoom={previewZoom} onZoomChange={handleManualZoom} isAutoFit={isAutoFit} onToggleAutoFit={toggleFit} onExportPng={(all) => { setExportScope(all?'all':'current'); setShowExportModal(true); }} onSave={handleSmartSave} isExporting={isExporting} showExportMenu={showExportMenu} setShowExportMenu={setShowExportMenu} exportMenuRef={exportMenuRef} showEditor={showEditor} onToggleEditor={() => setShowEditor(!showEditor)} canUndo={canUndo} canRedo={canRedo} onUndo={undo} onRedo={redo} />
